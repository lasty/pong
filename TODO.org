...
- Collision detection
  - [TODO] Clean up collision/physics code (repeated code a lot?)
  - [TODO] Fix bugs with collision/reflection handling
    - some balls still keep getting stuck on corners.
      - try one line at a time?
        - try closest line, or closest angle to normal?
      - try backing off if still collided after next step?
  - [DONE] force when collided
  - [TODO] Work out a way to signal collision from balls to blocks
    - event on collision ... start with just remove block
