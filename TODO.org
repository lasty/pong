

## Todo List
- command/key to enable debug lines
- [DONE] resize redraws border lines
- [DONE] Refactor and clean up code again  (move things to maths, collision routines out of game class)

- Gameish
  - [DONE] Arrange blocks in grid formation for test level
  - Change player to paddle rect
  - Paddle follow mouse cursor X position
  - Fire straight up from paddle
  - Eventually deflect on paddle

  - Remove ball from bottom of screen
    - Rework simple collision checks for out of bounds balls

- Sounds
  - [DONE] work out sdl_mixer
  - [DONE] generate sounds
  - [DONE] import sound samples into some kind of collection class
  - [DONE] work out a good way to play sounds from game class
  - Free sound chunks


## Todo Low Priority
- [TODO] Watch more of those videos
- [TODO] Investigate atom clang-format
- Particles class
- Do particles when block destroyed



## Done!



## Other Ideas

- Project limits, use no textures, only coloured vertexes?
